...
$path = $modx->getOption('subscribemeads.core_path',null,$modx->getOption('core_path').'components/subscribemeads/').'classes/';
$modx->getService('sm','SubscribeMe',$path);
require_once(dirname(dirname(dirname(__FILE__))).'/classes/paypal/paypal.class.php');

$debug = $modx->getOption('debug',$scriptProperties,$modx->getOption('subscribemeads.debug',null,false));
...
$p = array();
$p['currency_code'] = $modx->getOption('subscribemeads.currencycode',null,'USD');
$p['amount'] = $product->get('price');

$p['return_url'] = $modx->makeUrl($modx->getOption('return_id',$scriptProperties,$modx->getOption('subscribemeads.paypal.return_id')), '', '', 'full');
$p['cancel_url'] = $modx->makeUrl($modx->getOption('cancel_id',$scriptProperties,$modx->getOption('subscribemeads.paypal.cancel_id')), '', array('transid' => $subid), 'full');
$p['fail_id'] = $modx->getOption('fail_id',$scriptProperties,$modx->getOption('subscribemeads.paypal.fail_id'));
