...
class SubscribeMe {
    ...
    function __construct(modX &$modx,array $config = array()) {
        ...
        $basePath = $this->modx->getOption('subscribemeads.core_path',$config,$this->modx->getOption('core_path').'components/subscribemeads/');
        $assetsUrl = $this->modx->getOption('subscribemeads.assets_url',$config,$this->modx->getOption('assets_url').'components/subscribemeads/');
        $assetsPath = $this->modx->getOption('subscribemeads.assets_path',$config,$this->modx->getOption('assets_path').'components/subscribemeads/');
        $this->config = array_merge(array(
            ...
            'debug' => $this->modx->getOption('subscribemeads.debug',$config,false)
        ),$config);

        $this->modx->addPackage('subscribemeads',$this->config['model_path']);
        $this->modx->lexicon->load('subscribemeads:default');
    }
        ...
        if ($trans->save()) {
            ...
            if ($user instanceof modUser) {
                ...
                if ($up instanceof modUserProfile)
                    $upa = $up->toArray();
                $chunk = $this->modx->getOption('subscribemeads.email.confirmtransaction',null,'smConfirmTransactionEmail');
                $phs = array(
                ...
                $subject =  $this->modx->getOption('subscribemeads.email.confirmtransaction.subject',null,'Transaction processed for [[+product]] subscription');
                $subject = str_replace(
    ...
    public function sendNotificationEmail($type = '', smSubscription $subscription, modUser $user, smProduct $product, $transaction = '') {
        ...
        switch ($type) {
            case 'recurring_payment_profile_cancel':
                    $chunk = $this->modx->getOption('subscribemeads.email.confirmcancel',null,'smConfirmCancelEmail');
                    $subject = $this->modx->getOption('subscribemeads.email.confirmcancel.subject',null,'Your recurring payments profile for [[+product]] has been canceled.');
                break;

            case 'recurring_payment_skipped':
                    $chunk = $this->modx->getOption('subscribemeads.email.notifyskippedpayment',null,'smNotifySkippedPaymentEmail');
                    $subject = $this->modx->getOption('subscribemeads.email.notifyskippedpayment.subject',null,'A payment for your [[+product]] subscription has been skipped.');
                break;

            case 'recurring_payment_expired':
                    $chunk = $this->modx->getOption('subscribemeads.email.paymentexpired',null,'smPaymentExpiredEmail');
                    $subject = $this->modx->getOption('subscribemeads.email.paymentexpired.subject',null,'Your Recurring Payment for [[+product]] has expired.');
                break;

            case 'recurring_payment_cancelledbyadmin':
                    $chunk = $this->modx->getOption('subscribemeads.email.confirmcancel.admin',null,'smConfirmCancelAdminEmail');
                    $subject = $this->modx->getOption('subscribemeads.email.confirmcancel.admin.subject',null,'An administrator has cancelled your [[+product]] subscription.');
                break;

            case 'subscription_expired':
                    $chunk = $this->modx->getOption('subscribemeads.email.subscriptionexpired',null,'smSubscriptionExpiredEmail');
                    $subject = $this->modx->getOption('subscribemeads.email.subscriptionexpired.subject',null,'Your [[+product]] Subscription Expired.');
                break;
        }
